...
def load_5m_data(symbol, start_date, end_date):
    ...
    # Resampling can create empty 5m buckets overnight, on holidays, or
    # anywhere the source has no data. These are not candles and must not
    # appear on the chart.
...
def add_trade_box(fig, trade):
    ...
    if "stop_price" in trade.index and pd.notna(trade["stop_price"]):
        ...
        stop = entry - STOP_POINT if side == "LONG" else entry + STOP_POINT
    ...
    if "target_price" in trade.index and pd.notna(trade["target_price"]):
        ...
        target = entry + TAKE_PROFIT if side == "LONG" else entry - TAKE_PROFIT
...
def build_rangebreaks(index):
    """
    Remove every period for which there is no 5m candle.

    Using the actual missing timestamps rather than hard-coding market
    hours also removes weekends, holidays, overnight gaps, and any other
    missing-data periods automatically.
    """
    index = pd.DatetimeIndex(index).sort_values().drop_duplicates()
    ...
    return [dict(values=missing, dvalue=expected.total_seconds() * 1000)]


def make_chart(bars_5m, trades, symbol, start_date, end_date):
    fig = go.Figure()

    rangebreaks = build_rangebreaks(bars_5m.index)
